  ...
  return (
    <div style={{ textAlign: 'center', padding: '24px 0' }}>
      ...
      <motion.p
        ...
        style={{ fontSize: 36, fontWeight: 700, color: '#FF6B35', fontVariantNumeric: 'tabular-nums', letterSpacing: -1 }}
      >
        {timeLeft}
      </motion.p>
    </div>
...
export function CapsuleDetail() {
  ...
  if (!capsule) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', color: '#F0EAF5' }}>
        Capsule introuvable
      </div>
    )
  }
  ...
  const iconColor = capsule.unlockCondition.type === 'date' ? '#FF6B35'
    : capsule.unlockCondition.type === 'location' ? '#00C9A7' : '#7B61FF'
    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F' }}>
      ...
      <div style={{ position: 'relative', height: 320 }}>
        ...
        {capsule.status === 'locked' && !revealed && (
          <div style={{
            ...
            >
              <Lock size={56} color="#FF6B35" strokeWidth={1.5} />
            </motion.div>
          </div>
        )}
      ...
      <div style={{ padding: '20px 20px 40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div style={{
            background: capsule.status === 'unlocked' ? '#00C9A720' : '#FF6B3520',
            borderRadius: 8, padding: '4px 10px',
            display: 'flex', alignItems: 'center', gap: 6,
          }}>
            {capsule.status === 'locked' ? <Lock size={12} color="#FF6B35" /> : <span style={{ fontSize: 12 }}>✓</span>}
            <span style={{ fontSize: 11, color: capsule.status === 'unlocked' ? '#00C9A7' : '#FF6B35', fontWeight: 600 }}>
              {capsule.status === 'locked' ? 'Verrouillé' : 'Débloqué'}
            </span>
          </div>
        </div>

        <h1 style={{ fontSize: 26, fontWeight: 700, color: '#F0EAF5', marginBottom: 8, lineHeight: 1.2 }}>
          {capsule.title}
        </h1>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: 14, fontWeight: 600, color: '#F0EAF5' }}>
        ...
        <AnimatePresence mode="wait">
          {capsule.status === 'locked' && !revealed && (
            ...
            >
              {capsule.unlockCondition.type === 'date' && capsule.unlockCondition.date && (
                <div style={{ background: '#13121A', borderRadius: 16, marginBottom: 20 }}>
                  <Countdown date={capsule.unlockCondition.date} />
                </div>
              )}
              {capsule.unlockCondition.type === 'location' && (
                <div style={{
                  background: 'radial-gradient(circle at 50% 50%, #0D1B2A 0%, #0A0A0F 100%)',
                  ...
                  <svg width="100%" height="100%" style={{ position: 'absolute', opacity: 0.1 }}>
                    {[0,35,70,105].map(y => <line key={y} x1="0" y1={y} x2="430" y2={y} stroke="#F0EAF5" strokeWidth="0.5"/>)}
                    {[0,60,120,180,240,300,360,420].map(x => <line key={x} x1={x} y1="0" x2={x} y2="140" stroke="#F0EAF5" strokeWidth="0.5"/>)}
                  </svg>
                  <MapPin size={36} color="#00C9A7" />
              ...
              {capsule.unlockCondition.type === 'fragments' && (
                <div style={{ background: '#13121A', borderRadius: 16, padding: 20, marginBottom: 20 }}>
                  ...
                  <div style={{ background: '#0A0A0F', borderRadius: 8, height: 8, overflow: 'hidden' }}>
                    <motion.div
                      ...
                      style={{ height: '100%', background: 'linear-gradient(to right, #7B61FF, #A688FA)', borderRadius: 8 }}
                    />
                  </div>
                  <p style={{ fontSize: 12, color: '#7B61FF', marginTop: 8, fontWeight: 600 }}>
          ...
          {(capsule.status === 'unlocked' || revealed) && (
            ...
            >
              <div style={{
                background: 'linear-gradient(135deg, #00C9A715, #7B61FF10)',
                border: '1px solid rgba(0,201,167,0.2)',
                borderRadius: 20, padding: '20px',
                marginBottom: 16,
              }}>
                <p style={{ fontSize: 12, color: '#00C9A7', fontWeight: 600, marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>
                  ✨ Contenu révélé
                </p>
                <p style={{ fontSize: 15, color: '#F0EAF5', lineHeight: 1.7 }}>
